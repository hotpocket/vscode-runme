...
export class ConnectSerializer extends GrpcSerializerBase {
  ...
  protected async cacheNotebookOutputs(
    ...
    if (sid && docUri) {
      const relativePath = path.basename(docUri.fsPath)
      session = new es_proto.RunmeSession({
        id: sid,
        document: { relativePath },
      })
    }
    ...
    const plainReq = new es_proto.SerializeRequest({ notebook, options })
    const plainRes = this.client.serialize(plainReq)

    const maskedReq = new es_proto.SerializeRequest({ notebook: maskedNotebook, options })
    const masked = this.client.serialize(maskedReq).then((maskedRes) => {
  ...
  public static marshalNotebook(
    ...
    // the bulk copies cleanly except for what's below
    const notebook = new es_proto.Notebook(data as any)
  ...
  protected static marshalCellExecutionSummary(
    executionSummary: NotebookCellExecutionSummary | undefined,
  ): es_proto.CellExecutionSummary | undefined {
    if (!executionSummary) {
      return undefined
    }
    ...
    return new es_proto.CellExecutionSummary({
      success: success,
      timing: {
